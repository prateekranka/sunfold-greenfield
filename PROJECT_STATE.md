...
| **Current proof** | Cycle 01 provides the measured 43-degree Helios camera, closer full-ring framing, deeper fragment walls, brighter rough-metal materials, and a dominant solar objective. Cycle 02 adds native one-finger pan, pinch camera zoom, tap selection, and double-tap ground commands without page zoom or mouse regressions. Cycle 03 replaces smooth fragment wedges with four stepped armor bands, 26 instanced edge chunks, six inset deck plates, eight cracks, physical gold rails, and larger end sockets per sector. Its normal active-play sample rendered 600 frames in 10 seconds at 60.0 FPS, with 17.1 ms p95, 17.6 ms maximum, and no frame above 20 ms. The central-objective command still exposes a one-frame 66.6 ms first-use hitch. Cycle 02 remains the latest dev deployment: commit `5e94a46`, Worker `539e004a-b016-44d9-9c5e-1a23ee0ee914`, with matching hosted and local bundle hashes. |
| **Current mismatch** | Fragment surfaces and silhouettes now have real relief, but the solar core still lacks plasma structure and the resource islets and starfield remain visually quiet. The first full-group command toward the central objective can still miss the 16.67 ms frame budget while the large walk atlas becomes GPU-resident. The iPad proof uses the game's requestAnimationFrame counter; it is not a device GPU frame-time percentile export. |
...
| **Next correction** | Eliminate the first-command atlas upload hitch. Prewarm the shared Citizen and Guard runtime atlases before play, then repeat the central-objective and sustained-motion frame gates. |
